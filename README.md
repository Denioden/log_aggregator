...
```
Настроить базу данных. Описано в "Настройка setting.py"
...
__DATABASES__ - настройте в соответствии с настройками вашей базы данных.
...
- Укажите порт, для подключения к базе.
 ...
 __PARSER_LOG_PATH__ - укажите путь и маску ваших логов например: "home/user/logs/*.log".
  ...
  - __!!!!!!Внимание:__
        ...
        Система проверяет имя файла, первую и последнюю строку файла.
        Добовлять строки к ранее записанному файлу можно только в конец файла, в противном случае система будет работать не корректно.

        Если у файла новое имя сиcтема распознаёт его как новый файл, запишет данные и сообщит "Данные файла {file_name} записываются".

        Если у файла имя которое уже записано в базу, но новые данные (новая первая строка), система 
        сообщит - "Файл с именем {file_name}, уже записан в базу."

        Если в ранее записанный файл добавить новые строки система распознает это, обновит данные о файле, запишет новые данные 
...
__CRONJOBS__ - настройки параметров планового запуска скрипта, по умолчанию на запуск скрипта раз в 5 минут. Более детальные настройки можно посмотреть по ссылке: https://pypi.org/project/django-crontab/



## Запуск проекта

- Проверить что виртуальное окружение актевировано.
- Переходим в дерикторию, где находится файл manage.py
